...
def generate_progress_chart(
    ...
    if progress_history is None or not progress_history:
        plt.text(0.5, 0.5, "No progress data available", 
    ...
    if rewards_history:
        ...
        if len(rewards_history) != len(progress_history):
            ...
            if len(rewards_history) > len(progress_history):
                ...
                rewards_history.extend([last_reward] * (len(progress_history) - len(rewards_history)))
        ...
        if rewards_history:
            ...
            if reward_range > 0:
                normalized_rewards = [(r - min_reward) / reward_range for r in rewards_history]
            else:
                normalized_rewards = [0.5] * len(rewards_history)
                
            plt.plot(move_indices, normalized_rewards, 'r--', linewidth=1.5, label='Reward')
    
    # Highlight the solving move if provided
    if solved_at_move is not None and 0 <= solved_at_move < len(progress_history):
        plt.axvline(x=solved_at_move, color='g', linestyle='--', 
                   label=f'Solved at move {solved_at_move+1}')
        plt.plot([solved_at_move], [progress_history[solved_at_move]], 'g*', 
                markersize=15, label='Solution')
    ...
    if move_history:
        ...
        for i, move in enumerate(move_history):
            if move:  # Only annotate non-empty moves
                plt.annotate(move, (i, progress_history[i]), 
...
def generate_enhanced_cube_html(
    ...
    if progress_chart_base64:
        html += f'<img src="data:image/png;base64,{progress_chart_base64}" alt="Solving Progress Chart" style="max-width:100%;">'
    else:
...
def generate_face_html(face_name: str, face_colors: List[str]) -> str:
    ...
    for color in face_colors:
        bg_color = color_map.get(color[0], '#CCCCCC')  # Get first char of color, default to gray
        html += f'<div class="cubie" style="background-color: {bg_color};">{color}</div>'
...
if __name__ == "__main__":
    ...
    # Create example data
    example_moves = ["R", "U", "R'", "U'", "F", "R", "U", "R'", "U'", "F'", "U", "R", "U2", "R'"]
