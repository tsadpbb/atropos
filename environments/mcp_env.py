...
async def get_next_item(self):
    ...
    user_prompt_content = next_item_data["user_prompt_text"] # Key from your JSON
    expected_mcp_call_dict = next_item_data["expected_mcp_call"] # Key from your JSON

    prompt_messages = []
    # Your system prompt is added globally or handled by the tokenizer's chat template
    # If you had a per-item system message in your new dataset, you'd add it here.
    # For now, we assume the global system_prompt is sufficient.

    prompt_messages.append(
        frozenset({"role": "user", "content": user_prompt_content}.items())
    )

    # The "answer" is now the expected MCP tool call dictionary.
    # The score function will expect this to compare against the model's JSON output.
