...
class PokerEnv(BaseEnv):
    ...
    async def evaluate(self, *args, **kwargs):
        ...
        print(f"Starting evaluation on {sample_size} sampled examples (from {len(self.eval_indices)} total)...")
        print(f"Using temperature={eval_temperature}, max_tokens={eval_max_tokens}, correctness_threshold={correctness_threshold}")
        ...
        for idx in sampled_indices:
            ...
            try:
                ...
                # Score using reward function
                scores = self.reward_function.compute([text], winner_action=hand["winning_action"])
                score = scores[0] if scores else 0.0
        ...
        print(f"Evaluation Results (Current Run):")
        print(f"  Overall Accuracy: {current_accuracy:.4f} ({total_correct}/{total_examples})")
        ...
        for stage, stats in current_eval_stats.items():
            if stats["total"] > 0:
                ...
                print(f"  {stage}: Accuracy={stage_accuracy:.4f}, Avg Score={stage_avg_score:.4f}, Count={stats['total']}")

        # Log cumulative stats
        print(f"Cumulative Evaluation Results:")
        print(f"  Overall Accuracy: {cumulative_accuracy:.4f} ({cumulative_correct}/{cumulative_total})")
    ...
    async def wandb_log(self, wandb_metrics: Optional[Dict] = None):
        ...
        for action, count in self.stats["actions"].items():
            pct = count / self.stats["total_evaluated"] * 100 if self.stats["total_evaluated"] > 0 else 0
        ...
        if hasattr(self, 'eval_stats'):
            ...
            for stage, stats in self.eval_stats.items():
                if stats["total"] > 0:
                    stage_accuracy = stats["correct"] / stats["total"]
                    stage_avg_score = sum(stats["scores"]) / len(stats["scores"]) if stats["scores"] else 0
