...
class PokemonEnv(BaseEnv):
    ...
    async def setup(self):
        ...
        self.agent_player = GPTPlayer(model="gpt-4.1-nano")
        self.opponent_player = GPTPlayer(model="gpt-4.1-mini")

        if not self.server_configs:
            print(
                f"[{self.name}] No API server configurations found. LLM calls will fail."
            )
    ...
    async def collect_trajectories(
        ...
        group_data = ScoredDataGroup(
            raw_prompt=raw_prompts_group,
            raw_completion=raw_completions_group,
            prompt_tokens=prompt_tokens_group,
            completion_tokens=completion_tokens_group,
...
if __name__ == "__main__":
    # This makes the environment runnable with pydantic-cli
    # You can run: python environments/pokemon_environment.py serve --port 8001
    # And: python environments/pokemon_environment.py process --help
    PokemonEnv.cli()
