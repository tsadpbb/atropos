...
class Cube:
    ...
    def has_cross_on_face(self) -> bool:
        ...
        for face_idx, face in enumerate(self.cube):
            ...
            if (face[0][1] == center_color and  # Top middle
                face[1][0] == center_color and  # Left middle
                face[1][2] == center_color and  # Right middle
                face[2][1] == center_color):    # Bottom middle
                return True
...
# Import the curriculum learning module
from rubiks_cube_curriculum import RubiksCubeCurriculum, CurriculumLevel
# Import token-level rewards module
from rubiks_token_rewards import calculate_token_level_rewards, calculate_advantage_token_weights
# Import enhanced visualizer
from rubiks_enhanced_visualizer import save_enhanced_visualization
# Import solving strategies
...
class RubiksCubeEnvConfig(BaseEnvConfig):
    ...
    provide_solving_strategies: bool = True  # Include solving strategies in system prompt
    strategy_explanation_reward: float = 0.1  # Bonus reward for using strategy explanations
...
class RubiksCubeEnv(BaseEnv):
    def __init__(
        ...
        if self.use_curriculum:
            ...
            logger.info(f"Initialized curriculum learning at level {self.curriculum.current_level}")
        ...
        if self.generate_visualizations and self.visualizations_dir:
            ...
            logger.info(f"Created visualizations directory at {self.visualizations_dir}")
        ...
        self.tools = [
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "strategy": {
                            "type": "string",
                            "description": "Optional: The solving strategy you're using (e.g., 'Layer-by-Layer', 'CFOP', 'Beginner Method')"
                        }
                    },
                },
            }
    ...
    def _get_or_create_episode(self, seed: int) -> CubeState:
        if seed not in self.episodes:
            ...
            if self.use_curriculum:
                ...
                logger.debug(f"Using curriculum level {current_level.level} settings: "
                ...
                if self.provide_solving_strategies:
                    strategies_prompt = get_strategy_prompt_for_level(current_level.level)
            ...
            ep.curriculum_level = self.curriculum.current_level if self.use_curriculum else 0
    ...
    def _format_observation(self, cube_state: CubeState) -> str:
        ...
        scramble_info = ", ".join(cube_state.scramble_sequence) if cube_state.scramble_sequence else "None"
    ...
    def _calculate_cube_state_score(self, cube_state: CubeState) -> float:
        ...
        center_bonus = face_centers_correct * 0.05  # Small bonus for each correct center
        ...
        progress_reward = correctly_placed * cube_state.reward_per_correctly_placed_cubie
        ...
        corner_bonus = corners_correct * 0.03  # Bonus for each correctly positioned corner
        ...
        partial_face_bonus = partial_faces * 0.01  # Small bonus for partial face completion
        
        # Add base progress rewards
        score += progress_reward + center_bonus + face_bonus + cross_bonus + corner_bonus + edge_bonus + partial_face_bonus
        ...
        if cube_state.is_solved():
            ...
            efficiency_bonus = 2.0 * (1.0 - (cube_state.num_steps / cube_state.max_steps))
    ...
    def _parse_move(self, response: str, ep: CubeState = None) -> Tuple[Optional[str], Optional[str]]:
        ...
        strategy = parsed_args.get("strategy", "").strip() if isinstance(parsed_args, dict) else ""
            ...
            if match:
                ...
                if isinstance(thinking_content, str):
                    if thinking_content.strip():  # Not empty
                        thinking_quality = self._evaluate_thinking_quality(thinking_content, cube_state)
    ...
    def _evaluate_thinking_quality(self, thinking_content: str, cube_state: CubeState) -> float:
        """
        ...
        if cube_state.current_strategy:
            ...
            strategy_terms = [
                "layer", "cross", "corners", "edges", "algorithm",
                "orient", "permute", "f2l", "oll", "pll", "cfop",
                "beginner", "method", "technique", "sequence", "pattern"
            ]
            
            # Count strategy terms used
            term_count = sum(1 for term in strategy_terms if term in thinking_content.lower())
        ...
        if cube_state.cube is not None:
            ...
            color_terms = ["white", "yellow", "red", "orange", "blue", "green", "face", "center", "corner", "edge"]
            color_count = sum(1 for term in color_terms if term in thinking_content.lower())
        ...
        if re.search(r"(step|first|second|third|next|then|after)", thinking_content, re.IGNORECASE):
            score += 0.05
    ...
    async def wandb_log(self, wandb_metrics: Optional[Dict[str, float]] = None):
        ...
        if self.completed_episode_metrics_buffer:
            ...
            avg_scramble = (
                sum(m.get("scramble_length", 0) for m in self.completed_episode_metrics_buffer)
                / num_episodes
            )
            ...
            if self.use_curriculum:
                ...
                wandb_metrics[f"{prefix}_curriculum/level"] = curriculum_metrics["curriculum_level"]
                wandb_metrics[f"{prefix}_curriculum/success_rate"] = curriculum_metrics["level_success_rate"]
                wandb_metrics[f"{prefix}_curriculum/progress_to_next"] = curriculum_metrics["progress_to_next_level"]
                wandb_metrics[f"{prefix}_curriculum/solved_count"] = curriculum_metrics["level_solved_count"]
                wandb_metrics[f"{prefix}_curriculum/episodes"] = curriculum_metrics["level_episodes"]
                ...
                for level, count in level_episodes.items():
                    ...
                    wandb_metrics[f"{prefix}_curriculum/level_{level}_success_rate"] = success_rate
    ...
    def _generate_episode_visualization(self, ep: CubeState, is_best: bool = False) -> Optional[str]:
    ...
    def cli(cls):
        ...
        try:
            ...
            if hasattr(cls, 'config') and cls.config.generate_visualizations and cls.config.visualizations_dir:
                print("\nGenerating consolidated report of all solving attempts...")
                report_path = generate_consolidated_report_from_dir(cls.config.visualizations_dir)
