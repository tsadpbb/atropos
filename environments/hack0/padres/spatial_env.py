...
class MVPPhysicsSimulator:
    ...
    def _add_object(self, obj_state: ObjectState):
        ...
        if obj_state.type == "cube":
            shape_id = p.createCollisionShape(p.GEOM_BOX, halfExtents=half_extents, physicsClientId=self.client_id)
        elif obj_state.type == "sphere":
            shape_id = p.createCollisionShape(p.GEOM_SPHERE, radius=half_extents[0], physicsClientId=self.client_id)
        else:
            print(f"Warning: Unsupported object type '{obj_state.type}' for object ID '{obj_state.id}'")
            return
    ...
    def move_object(self, object_id: str, target_position: List[float], target_orientation_quaternion: Optional[List[float]] = None):
        if object_id in self.objects_pb_ids:
            body_id = self.objects_pb_ids[object_id]
            if target_orientation_quaternion is None:
                _, current_orientation = p.getBasePositionAndOrientation(body_id, physicsClientId=self.client_id)
                target_orientation_quaternion = list(current_orientation)
            p.resetBasePositionAndOrientation(body_id, target_position, target_orientation_quaternion, physicsClientId=self.client_id)
        else:
    ...
    def get_current_state_for_visualization(self) -> List[Dict[str, Any]]:
        ...
        for obj_id, body_id in self.objects_pb_ids.items():
            pos, orn_quat = p.getBasePositionAndOrientation(body_id, physicsClientId=self.client_id)
...
async def notify_visualization_clients(scene_state: List[Dict[str, Any]]):
    if connected_visualization_clients:
        ...
        await asyncio.gather(*[client.send(message) for client in connected_visualization_clients])
...
async def visualization_websocket_handler(websocket):
    ...
    print(f"Visualization client connected: {websocket.remote_address} (Total: {len(connected_visualization_clients)})")
    try:
        if global_physics_simulator_instance:
            initial_state = global_physics_simulator_instance.get_current_state_for_visualization()
            await websocket.send(json.dumps({"type": "initial_scene", "payload": initial_state}))
            ...
            try:
                ...
                if command == "next_llm_task":
                    ...
                    if shared_demo_runner_instance:
                        ...
                        asyncio.create_task(shared_demo_runner_instance.run_single_turn_demo(use_real_llm=True))
                    else:
                        print("Error: shared_demo_runner_instance not found to execute 'next_llm_task'")
                else:
        ...
        print(f"Visualization client disconnected. (Total: {len(connected_visualization_clients)-1})")
    except Exception as e:
...
class MVPDemoRunner:
    ...
    async def run_single_turn_demo(self, use_real_llm: bool = True):
        ...
        print(f"Achieved Distance: {result['metadata']['final_distance']:.2f} (Target: {result['metadata']['target_distance']:.2f})")
        return result

async def process_mode(args):
    print(f"Running in 'process' mode: generating {args.num_turns} trajectories to {args.output_file}")
    ...
    try:
        for i in range(args.num_turns):
            ...
            turn_result = await demo_runner.run_single_turn_demo() # Assumes run_single_turn_demo uses real LLM by default now
            ...
            if wandb_is_initialized and wandb.run:
                wandb_log_data = {
                    ...
                    "side_condition_met": int(turn_result.get("metadata", {}).get("side_condition_met", False)),
                }
        ...
        if wandb_is_initialized and wandb.run: 
            ...
            print("Processing complete. (W&B was not fully initialized or did not start a run)")
...
async def server_mode():
    ...
    print(f"Visualization WebSocket Server started on ws://localhost:8765")
...
async def main():
    ...
    process_parser.add_argument('--num_turns', type=int, default=5, help='Number of trajectory turns to generate')
    process_parser.add_argument('--output_file', type=str, default='trajectories.jsonl', help='File to save trajectory data')
