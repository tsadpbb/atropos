...
DOMAINS = [
    "e-commerce", "finance", "healthcare", "education", "real estate", 
    "social media", "marketing", "transportation", "manufacturing", 
    "hospitality", "technology", "energy", "retail", "agriculture",
    "media", "telecommunications", "insurance", "entertainment"
]

# Metric type templates
METRIC_TYPES = [
    "revenue", "user growth", "conversion rate", "customer acquisition cost",
    "average order value", "customer lifetime value", "page views", "bounce rate",
    "churn rate", "active users", "session duration", "retention rate",
    "engagement rate", "click-through rate", "inventory turnover", "profit margin",
    "return on investment", "customer satisfaction", "net promoter score",
    "sales growth", "market share", "employee productivity", "website traffic",
    "operational costs", "customer support tickets", "app downloads",
    "subscription renewals", "delivery time", "error rate", "uptime percentage"
]

# Time periods for metrics
TIME_PERIODS = [
    "this month", "this quarter", "this year", "last 30 days", 
    "last 7 days", "last 24 hours", "year-to-date", "Q1 2023",
...
ICONS = [
    "trending-up", "trending-down", "users", "dollar", "shopping-cart",
    "bar-chart", "percent", "activity", "alert-circle", "clock",
    "heart", "globe", "mail", "phone", "eye", "tag", "star",
    "bell", "download", "upload", "refresh", "check-circle",
    "shield", "pie-chart", "layers", "calendar", "thumbs-up"
]
...
class TrendModel(BaseModel):
    ...
    isPositive: bool = Field(..., description="Whether the trend is positive or negative")

class MetricCardProps(BaseModel):
    model_config = ConfigDict(extra="forbid", exclude_none=True)
    title: str = Field(..., description="The metric name/label")
    value: str = Field(..., description="The metric value (formatted as appropriate string)")
    description: Optional[str] = Field(None, description="Additional context about the metric")
    trend: Optional[TrendModel] = Field(None, description="Trend information including value and direction")
    icon: Optional[str] = Field(None, description="Name of the icon to display")
    className: Optional[str] = Field(None, description="CSS class name for custom styling")

class MetricCardComponent(BaseModel):
    model_config = ConfigDict(extra="forbid", exclude_none=True)
    componentType: str = Field(..., description="Must be exactly 'metricCard'")
    props: MetricCardProps = Field(..., description="The properties for the metric card component")
...
class MetricCardEnv(BaseEnv):
    ...
    async def evaluate_metric_card(self, json_text):
        ...
        try:
            ...
            if (isinstance(data, dict) and 
                data.get("componentType") == "metricCard" and 
                "props" in data and 
                "title" in data["props"] and 
                "value" in data["props"]):
                evaluation["has_required_fields"] = True
            
            # Validate against schema
            try:
                metric_card = MetricCardComponent(**data)
                ...
                if isinstance(value, str):
                    ...
                    if (("$" in value or "€" in value or "£" in value) or
                        ("%" in value) or
                        ("," in value and any(c.isdigit() for c in value))):
                        formatting_score += 1
                
                # Check trend formatting
                if "trend" in props and isinstance(props["trend"], dict):
                    if (isinstance(props["trend"].get("value"), (int, float)) and
                        isinstance(props["trend"].get("isPositive"), bool)):
                        formatting_score += 1
                ...
                evaluation["overall_quality"] = (evaluation["schema_valid"] * 0.5 + 
    ...
    async def collect_trajectories(
        ...
        for item in to_score:
            ...
            scores["scores"].append(random.uniform(-0.5, 0.5))  # Random scores to avoid the "all same" check
    ...
    async def score(
        ...
        for idx, item in enumerate(rollout_group_data):
            ...
            scores["scores"].append(-1.0 + idx * 0.5)  # Will give different scores for each item
