...
def filter_and_format_for_finetune(input_path: str, output_path: str, score_threshold: float = 0.0, debug: bool = False):
    filtered_count = 0
    total_count = 0
        ...
        for line_num, line in enumerate(infile, 1):
            ...
            try:
                ...
                if score <= score_threshold:
                    if debug:
                        print(f"Line {line_num}: Score {score} is below threshold {score_threshold}")
                ...
                try:
                    ...
                    elif isinstance(messages, tuple) or (isinstance(messages, list) and len(messages) > 0 and isinstance(messages[0], tuple)):
                        # Format where messages are tuples
                        system_msg = messages[0]["content"] if isinstance(messages[0], dict) else messages[0][1]
                        user_msg = messages[1]["content"] if isinstance(messages[1], dict) else messages[1][1]
                        assistant_msg = messages[2]["content"] if len(messages) > 2 and isinstance(messages[2], dict) else messages[2][1] if len(messages) > 2 else ""
                    else:
                        if debug:
                            print(f"Line {line_num}: Unsupported message format: {type(messages[0])}")
                        skipped_due_to_structure += 1
                        continue
                    ...
                    if "```json" in completion and "```" in completion.split("```json", 1)[1]:
                        completion = completion.split("```json", 1)[1].split("```", 1)[0].strip()
                    elif "```" in completion and "```" in completion.split("```", 1)[1]:
                        completion = completion.split("```", 1)[1].split("```", 1)[0].strip()
                    
                    # Ensure the completion is valid JSON for our metric card
                    try:
                        completion_json = json.loads(completion)
                        # Verify it has the required structure for a metric card
                        if not (isinstance(completion_json, dict) and 
                                completion_json.get("componentType") == "metricCard" and
                                "props" in completion_json and
                                "title" in completion_json["props"] and
                                "value" in completion_json["props"]):
                            if debug:
                    ...
                    json.dump({"prompt": prompt, "completion": completion}, outfile, ensure_ascii=False)
                    outfile.write("\n")
                    filtered_count += 1
    ...
    print(f"Finished processing. Kept {filtered_count} out of {total_count} examples with score > {score_threshold}.")
    ...
    if filtered_count == 0 and total_count > 0:
        ...
        print("You can use --score_threshold -1.0 to see all examples regardless of score.")
        ...
        try:
            ...
            if scores:
                ...
                for range_name, count in ranges.items():
                    if count > 0:
                        print(f"{range_name}: {count} examples ({count/len(scores)*100:.1f}%)")
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            try:
                ...
                if "results" in data:
                    print("This appears to be a consolidated results file, not a rollouts file")
                    ...
                    if data['results']:
                        ...
                        if "json_text" in first_result:
                            print(f"JSON text sample: {first_result['json_text'][:100]}...")
                            
                            try:
                                json_obj = json.loads(first_result['json_text'])
                                print(f"Valid JSON object with keys: {list(json_obj.keys())}")
                            except:
                                print("JSON text is not valid JSON")
                    
                    print("\nThis file is not in the expected format for the script.")
                    print("The script expects individual JSONL records, not a consolidated JSON file.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter and convert JSONL eval file for fine-tuning.")
    ...
    parser.add_argument("--score_threshold", type=float, default=0.0, help="Minimum score to keep an example (default: 0.0)")
    parser.add_argument("--debug", action="store_true", help="Enable debug output")
    parser.add_argument("--analyze", action="store_true", help="Analyze file structure without processing")
    args = parser.parse_args()
    
    if args.analyze:
        analyze_raw_file(args.input_path)
    else:
        filter_and_format_for_finetune(args.input_path, args.output_path, args.score_threshold, args.debug)
