...
def extract_training_examples(input_file: str, output_file: str, score_threshold: float = -1.0):
    ...
    with open(input_file, 'r') as f_in, open(output_file, 'w') as f_out:
        for line in f_in:
            ...
            try:
                ...
                system_match = re.search(r'<\|start_header_id\|>system<\|end_header_id\|>\s*(.*?)<\|eot_id\|>', raw_messages, re.DOTALL)
                user_match = re.search(r'<\|start_header_id\|>user<\|end_header_id\|>\s*(.*?)<\|eot_id\|>', raw_messages, re.DOTALL)
                assistant_match = re.search(r'<\|start_header_id\|>assistant<\|end_header_id\|>\s*(.*?)<\|eot_id\|>', raw_messages, re.DOTALL)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract training examples from metric card rollouts")
