...
from environments.hack0.protein_design_env.tool_definitions import ALL_TOOLS_LIST
from environments.hack0.protein_design_env.tool_executor import ToolExecutor
from environments.hack0.protein_design_env.utils.pdb_utils import get_pdb_chain_details
from environments.hack0.protein_design_env.prompts import SYSTEM_PROMPT, construct_user_prompt
...
def load_target_binder_pairs(dataset_name: str, target_col: str, binder_col: str, split: str = "train") -> Dataset:
    ...
    try:
        ...
        if actual_target_col in ds.column_names and actual_binder_col in ds.column_names:
            ...
            logger.error(f"Could not find expected columns in dataset. Available columns: {ds.column_names}")
            raise ValueError(f"Dataset {dataset_name} doesn't have the expected columns. Please check your dataset configuration.")
...
class BinderBenchConfig(BaseEnvConfig):
    ...
    max_retries_per_internal_step: int = Field(100, description="Max retries for a failed tool call within a workflow step (0 means no retries).")
...
class BinderBenchEnv(BaseEnv):
    ...
    async def _execute_tool(self, tool_name: str, args: Dict, workflow_state: Dict) -> Dict:
        ...
        execution_result_package = await self.tool_executor.dispatch_tool_call(tool_name, args, workflow_state)
        ...
        if state_updates:
            ...
            logger.debug(f"Workflow {workflow_state['item_id']}: State updated with keys: {list(state_updates.keys())}")
    ...
    def _score_trajectory(self, turn_messages: List[Message], workflow_state: Dict) -> Dict[str, float]:
        ...
        if internal_step < 3:
            ...
            if workflow_state.get("complex_evaluated") and last_tool_success and called_tool_name == expected_tool_for_step:
                ...
                detailed_scores["overall_reward"] = 0.0
                logger.warning(f"Workflow {workflow_state['item_id']}, Step {internal_step} (AF2-Multimer): Evaluation failed or wrong tool. Reward: 0.0. Last tool success: {last_tool_success}, Called: {called_tool_name}")
    ...
    async def add_rollouts_for_wandb(self,
        ...
        if isinstance(designed_binder_data, list) and designed_binder_data:
            ...
            preview_text = first_chain_seq[:30] + "..." if len(first_chain_seq) > 30 else first_chain_seq
            ...
            binder_preview = designed_binder_data[:30] + "..." if len(designed_binder_data) > 30 else designed_binder_data
    ...
    async def create_rollout_table(self, wandb_metrics: Dict) -> Dict:
        ...
        if hasattr(self, "rollouts_for_wandb") and self.rollouts_for_wandb:
            columns = ["Item ID", "Target (Preview)", "Designed Binder (Preview)",
                      "Final pLDDT", "Cumulative Reward", "Last Turn Messages"]
