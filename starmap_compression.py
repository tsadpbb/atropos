...
class StarMapCompressionEnv(BaseEnv):
    def __init__(self, data_path, views_path):
        ...
        logging.info(f"Original data shape: {self.original_data.shape}, Views shape: {self.views.shape}")
        logging.info(f"Original data range: min={self.original_data.min(axis=0)}, max={self.original_data.max(axis=0)}")
        ...
        logging.info(f"PCA data shape: {self.pca_data.shape}, PCA views shape: {self.pca_views.shape}")

        # Compute adaptive density threshold
        data_range = self.original_data.max(axis=0) - self.original_data.min(axis=0)
        cell_volume = np.prod(data_range[data_range > 0]) if np.any(data_range > 0) else 1.0
        ...
        self.view_radius = min(valid_range) * 2.0 if len(valid_range) > 0 else 50.0  # ~53.4 to cover min view distance
        self.partition_methods = [self.view_radius * 0.5, self.view_radius, self.view_radius * 1.5]
        logging.info(f"View radius: {self.view_radius}, Partition methods: {self.partition_methods}")
    ...
    def _density_sample(self, data, sample_fraction=0.1, radius=50.0):
        if len(data) == 0:
            return data
        tree = cKDTree(data)
        density = np.array([len(tree.query_ball_point(point, radius)) for point in data])
        density = density / (density.sum() + 1e-10)
        num_samples = max(1, int(len(data) * sample_fraction))
        indices = np.random.choice(len(data), size=num_samples, p=density, replace=False)
        return data[indices]
    ...
    def _build_octree(self, data, min_points=2, max_depth=5, density_threshold=None):
        ...
        logging.info(f"Octree level {5-max_depth}: density={density}, threshold={density_threshold}")
        ...
        for octant in octants:
            if len(octant) > 0:
                ...
                subtree = self._build_octree(octant_data, min_points, max_depth - 1, density_threshold)
                if len(subtree) > 0:
    ...
    def step(self, action):
        ...
        total_points = sum(np.sum(np.sqrt(np.sum((self.data - v) ** 2, axis=1)) < self.view_radius) for v in self.views)
        quality = np.mean([np.min(np.sum((self.data - v) ** 2, axis=1)) for v in self.views]) if len(self.data) > 0 else 0
        reward = -avg_data_size / 1000 + 5 * len(self.data) / len(self.original_data) + total_points / len(self.original_data) - quality / 1e6
        done = self.step_count >= self.max_steps
        logging.info(f"Step reward: data_size={avg_data_size}, points={len(self.data)}, total_points={total_points}, quality={quality}, reward={reward}")
    ...
    def _evaluate_partition(self):
        ...
        for view in self.views:
            ...
            logging.info(f"View: points_in_view={points_in_view}, num_cells={num_cells}, cell_size={cell_size}, data_size={data_size}, cell_overhead={cell_overhead}")

        avg_data_size = total_size / len(self.views) if len(self.views) > 0 else 0
        logging.info(f"Avg data size: {avg_data_size} bytes for grid size {cell_size}, Total points in views: {total_points}")
    ...
    def _recompress_data(self, grid_size):
        ...
        logging.info(f"Recompress: grid_size={grid_size}, density_threshold={density_threshold}, max_depth={max_depth}, min_points={min_points}, bits={bits}, sample_fraction={sample_fraction}")
        octree_data = self._build_octree(self.pca_data, min_points=min_points, max_depth=max_depth, density_threshold=density_threshold)
        ...
        if len(compressed_data) > 0:
            ...
            indices = np.random.choice(len(compressed_data), size=num_samples, replace=False)
            compressed_data = compressed_data[indices]
        logging.info(f"Recompressed data shape for grid size {grid_size}: {compressed_data.shape}")
    ...
    def run_rl_step(self, timeout_seconds=60):
        ...
        def step_with_action(action):
            try:
                ...
                logging.info(f"Action {action} (grid size {self.partition_methods[action]}): reward={reward}")
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for action, future in enumerate(futures):
                try:
                    ...
                    logging.warning(f"Action {action} timed out after {timeout_seconds} seconds")
        ...
        for reward, action in rewards:
            if reward > best_reward or (reward == best_reward and np.random.random() < 0.5):  # Random tiebreaker
                best_reward = reward
                best_action = action

        self.step(best_action)
        result = self.evaluate()
        logging.info(f"After RL Step: Chose grid size {self.partition_methods[best_action]}, avg_data_size = {result['avg_data_size']} KB, Reward: {best_reward}")